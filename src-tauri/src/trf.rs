...
pub fn get_partial_players_lines(
    players: &[(u16, Player)],
    rounds: &[Round],
...
pub fn get_players_lines(
    players: &[(usize, usize, Player)],
    rounds: &[Round],
    connection: &Connection,
) -> Result<Vec<String>, Error> {
    let lines = players
        .iter()
        .map(|(starting_rank, rank, player)| {
            let player_data = format!(
                "001 {:>4} {:>1}{:>3} {:>33} {:>4} {:>3} {:>11} {:>10} {:>4.1} {:>4}",
                starting_rank,
                "",
                if matches!(player.title, Some(_)) {
                    player.title.as_ref().unwrap().to_string()
                } else {
                    String::from("")
                },
                player.name,
